...
## Header action URL closures must read $this->tableFilters lazily
Header actions are cached during boot, before InteractsWithTable fills $this->tableFilters. An arrow fn that captures a computed $queryParams array at getHeaderActions() time bakes in empty/null filter values, so the Cetak/Export URLs drop the applied filters. Instead pass a closure that calls a method reading $this->tableFilters at getUrl() time (render), e.g. ->url(fn () => $this->getReportUrl('...')) with getReportUrl() reading $this->tableFilters ?? [].
