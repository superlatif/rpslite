...
<head>
    ...
    <title>Laporan Retur Pembelian {{ $header->trs_number }}</title>
    <style>
...
<body>
    @php
        ...
        $totalSubtotal = $details->sum(fn ($item): float => (float) $item->subtotal);
    @endphp

    @if (! $autoPrint)
        <div class="toolbar">
            <a class="btn" href="{{ route('filament.admin.retur-pembelian.cetak', $header) }}" target="_blank">Cetak</a>
            <a class="btn" href="{{ route('filament.admin.retur-pembelian.export', $header) }}">Export Excel</a>
        </div>
    @endif

    <div class="report">
        <div class="center brand">{{ config('app.name') }}</div>
        <div class="center title">LAPORAN RETUR PEMBELIAN</div>
        ...
        <table class="info">
            ...
            <tr>
                ...
                <td>: {{ filled($supplier?->phone) ? $supplier->phone : '---' }}</td>
            </tr>
            <tr>
                <td class="label">No. Faktur Beli</td>
                <td>: {{ $header->sourcePurchase?->trs_number ?? '-' }}</td>
            </tr>
        </table>
        <hr class="separator">
