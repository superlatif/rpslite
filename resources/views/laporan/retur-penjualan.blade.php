...
<head>
    ...
    <title>Laporan Retur Penjualan {{ $header->trs_number }}</title>
    <style>
...
<body>
    ...
    @if (! $autoPrint)
        <div class="toolbar">
            <a class="btn" href="{{ route('filament.admin.retur-penjualan.cetak', $header) }}" target="_blank">Cetak</a>
            <a class="btn" href="{{ route('filament.admin.retur-penjualan.export', $header) }}">Export Excel</a>
        </div>
    @endif

    <div class="report">
        <div class="center brand">{{ config('app.name') }}</div>
        <div class="center title">LAPORAN RETUR PENJUALAN</div>
        ...
        <table class="info">
            ...
            <tr>
                <td class="label">No. Faktur Jual</td>
                <td>: {{ $header->sourceSale?->trs_number ?? '-' }}</td>
            </tr>
        </table>
        <hr class="separator">
